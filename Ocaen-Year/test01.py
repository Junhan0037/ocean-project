...
def filtering_data_date(input_file, output_file):
    ...
    for date_time in tqdm_notebook(data_frame['Observation Date']):
        data_frame['Observation Date'][i] = parse(date_time).strftime('%Y-%m')
        i = i + 1
    ...
    for date_time in data_frame['Observation Date']:
        if parse(date_time).strftime('%m') == '08':
            date_index.append(i)
        i += 1
    data_frame = data_frame.iloc[date_index, :]
...
if __name__ == "__main__":
    ...
    filtering_data_date('ocean_all_data.csv', 'filtering_data_year.csv')